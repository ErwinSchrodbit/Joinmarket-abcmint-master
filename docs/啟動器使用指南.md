...
## 下載與校驗
- 可執行檔位置：`joinmarket_abcmint\dist\JoinMarket-ABCMint-LauncherV1.0.0.exe`
- 使用 PowerShell 進行 SHA256 校驗：
  ```powershell
  Get-FileHash -Path "c:\Users\mark\Desktop\GUI开发中\joinmarket_abcmint\dist\JoinMarket-ABCMint-LauncherV1.0.0.exe" -Algorithm SHA256 | Select-Object -ExpandProperty Hash
  ```
- 或使用 `certutil`：
  ```powershell
  certutil -hashfile "c:\Users\mark\Desktop\GUI开发中\joinmarket_abcmint\dist\JoinMarket-ABCMint-LauncherV1.0.0.exe" SHA256
  ```
...
- 連線失敗：確認節點已啟動並啟用 RPC、埠正確、`127.0.0.1` 可存取、防火牆未阻擋。
